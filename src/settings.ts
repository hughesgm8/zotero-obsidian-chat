...
import { DEFAULT_SUMMARY_SECTIONS } from "./types";
...
export class ZoteroMCPSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		// --- Zotero MCP Section ---
		new Setting(containerEl).setName("Zotero MCP Server").setHeading();
		...
		new Setting(containerEl).setName("Smart Import").setHeading();
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Describe your research focus so the AI can assess how each imported paper relates to your work."
			)
			.addTextArea((text) =>
				text
					.setPlaceholder(
		...
		new Setting(containerEl).setName("Summary sections").setHeading();
		containerEl.createEl("p", {
			text: "The sections generated by the AI when importing a paper. Add, remove, or edit sections to suit your workflow. Each section name becomes a heading in the output note.",
			cls: "setting-item-description",
		});

		const sectionsContainer = containerEl.createDiv("zotero-summary-sections");

		const renderSections = () => {
			sectionsContainer.empty();
			this.plugin.settings.summarySections.forEach((section, index) => {
				const card = sectionsContainer.createDiv("zotero-summary-section-card");

				const nameRow = card.createDiv("zotero-summary-section-name-row");
				const nameInput = nameRow.createEl("input", { type: "text" });
				nameInput.value = section.name;
				nameInput.placeholder = "Section name";
				nameInput.addEventListener("change", async () => {
					this.plugin.settings.summarySections[index].name = nameInput.value;
					await this.plugin.saveSettings();
				});

				const upBtn = nameRow.createEl("button", { text: "↑", cls: "zotero-summary-section-move" });
				upBtn.disabled = index === 0;
				upBtn.addEventListener("click", async () => {
					const arr = this.plugin.settings.summarySections;
					[arr[index - 1], arr[index]] = [arr[index], arr[index - 1]];
					await this.plugin.saveSettings();
					renderSections();
				});

				const downBtn = nameRow.createEl("button", { text: "↓", cls: "zotero-summary-section-move" });
				downBtn.disabled = index === this.plugin.settings.summarySections.length - 1;
				...
				const deleteBtn = nameRow.createEl("button", { text: "Delete", cls: "zotero-summary-section-delete" });
				deleteBtn.addEventListener("click", async () => {
					this.plugin.settings.summarySections = this.plugin.settings.summarySections.filter((_, i) => i !== index);
					await this.plugin.saveSettings();
					renderSections();
				});

				card.createEl("label", { text: "Instructions", cls: "zotero-summary-section-label" });
				const textarea = card.createEl("textarea", { cls: "zotero-summary-section-textarea" });
				textarea.value = section.instructions;
				textarea.placeholder = "Describe what the AI should write in this section…";
				textarea.rows = 3;
				textarea.addEventListener("change", async () => {
					this.plugin.settings.summarySections[index].instructions = textarea.value;
					await this.plugin.saveSettings();
				});
			});
		};
		...
		const sectionActions = containerEl.createDiv("zotero-summary-section-actions");

		const addBtn = sectionActions.createEl("button", { text: "+ Add section", cls: "mod-cta" });
		addBtn.addEventListener("click", async () => {
			this.plugin.settings.summarySections.push({ name: "New section", instructions: "" });
			await this.plugin.saveSettings();
			renderSections();
		});

		const resetBtn = sectionActions.createEl("button", { text: "Reset to defaults" });
		resetBtn.addEventListener("click", async () => {
			this.plugin.settings.summarySections = DEFAULT_SUMMARY_SECTIONS.map(s => ({ ...s }));
			await this.plugin.saveSettings();
			renderSections();
		});
